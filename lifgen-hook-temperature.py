...
trust_remote_code=True, token=hf_token)
     ...
     EngineID = f"mistralai/Mistral-7B-Instruct-v0.1 ({DEVICE.upper()})"  # Updated EngineID "#f"DeepSeek R1 1.5B Qwen-Distil Greedy ({DEVICE.upper()})"  # Updated EngineID
...
slice_start_index:slice_end_index]
                 ...
                 with torch.no_grad():
                     outputs = model.generate(
                            input_tensor_dev,
                            max_new_tokens=1,
                            min_new_tokens=1,  # Require exactly 1 new token
                            output_scores=True,
                            return_dict_in_generate=True,
                            do_sample=True,  # Enable sampling
                            temperature=100.0,  # Set temperature to maximum (very high value)
                            top_k=None,
                            top_p=None,
                            pad_token_id=tokenizer.pad_token_id,
                            # Remove beam-related params when sampling
